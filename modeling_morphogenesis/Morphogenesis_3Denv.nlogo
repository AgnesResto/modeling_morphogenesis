...
to make-glass3D
  ...
  ask patches [
   sprout 1
   [ifelse (pycor >= min-pycor and pycor < min-pycor + gel-bed-thickness)
     [set breed glass
       set color cyan]
     [ifelse pycor = min-pycor + gel-bed-thickness
      [ifelse (percent-cells > random 100)
       [set breed cells
        set ss? False
       set shape atom-shape
       set color red]
       [ifelse not any? cells-here and overlay-matrix-percentage > random 100
          [set breed matrix
           set color white]
            [die]]]
       [ifelse not any? cells-here and overlay-matrix-percentage > random 100 and (pycor >= min-pycor + gel-bed-thickness) and (pycor <= min-pycor + gel-bed-thickness + overlay-thickness)
         [set breed matrix
              set color white]
      [die]]
    ]
  ...
  ask patches [
   ...
   [ifelse (pycor >= min-pycor and pycor < min-pycor + gel-bed-thickness)
     ...
     [ifelse pycor = min-pycor + gel-bed-thickness
       ...
       set color red]
       [die]]
       [ifelse (pycor > min-pycor + gel-bed-thickness and pycor < min-pycor + gel-bed-thickness + overlay-thickness)
       [ifelse (overlay-matrix-percentage > random 100)
       [set breed matrix
        set color white]
       [die]
          ]
