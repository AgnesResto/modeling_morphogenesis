...
;    stop
  ...
  [if steady-state? = True or ticks >= 300 [
    ask patches [
      ;; define neighborhood of patches
      ifelse pxcor mod 2 = 0 [
        set patch-neighbors-6 turtles-on patches at-points [[0 1] [1 0] [1 -1] [0 -1] [-1 -1] [-1 0]]
...
;    set neighbors-6 turtles-on patches at-points [[0 1] [1 1] [1  0] [0 -1] [-1  0] [-1 1]] ]
  ...
  [
    ...
    [
      ifelse count turtles-on neighbors-6 = 0 ;; if you don't have any neighbors you die
      [die]
      [
        ifelse count cells-on neighbors-6 = 6
        [
          ifelse not any? link-neighbors ;; if all of your neighbors are cells but you aren't linked, initiate lumenogenesis
          [
            create-cell-links-with (cells-on neighbors-6) [set color blue]
            ask (cells-on neighbors-6) [ create-cell-links-with (cells-on neighbors-6) [set color blue] ]
            push-cell-to-matrix
            ask cells [refresh-neighbors]
          ]
          [
            ask (cells-on neighbors-6) [ create-cell-links-with (cells-on neighbors-6) [set color blue] ]
            die ;; cell surrounded by other cells but has already polarized? Possible?
          ]
        ]
        [
          ifelse count matrix-on neighbors-6 = 6
          [
            hatch-matrix 1 [set color white] ;; if you are surrounded by matrix, die
            die
          ]
          [ifelse count link-neighbors in-radius 1.5 >= count cells-on neighbors-6 or count link-neighbors = 0 ;; take the following actions only if you don't have links or if you are only surrounded by the cells of your cyst. This prevents different cysts from merging
          [
            ifelse (count matrix-on neighbors-6 > 0) and (count cells-on neighbors-6 > 0) and (count matrix-on neighbors-6 + count cells-on neighbors-6) = 6 ;; two types of neighbors: matrix and cells
            [
              ifelse count cells-on neighbors-6 < 5 ;; this rule is here because I was assuming that a cyst can't form with less than 5 cells but it may be removed in the future
              [
                  ifelse count cells-on neighbors-6 <= 2 and not any? link-neighbors
                  [
                  ...
                  [
                    ifelse count cells-on neighbors-6 >= 3
                    [
...
;;                      create-cell-links-with (cells-on neighbors-6) [set color blue]
;                      ifelse pxcor mod 2 = 0
;                      [rule5-even] ;; move to matrix next to cell
;                      [rule5-odd]
;                    ]
;                  ]
;                  [
;                    ;; cell surrounded by matrix and cells that can no longer divide
;                    ;; do nothing
;                  ]
;                ]
;                [
;                  ;; cell surrounded by cells and matrix that already has some cell links
;                  ;; there is probably some empty space in-radius 2 or 2.5 (thick cyst)
;                ]
              ]
              [
                ;; more than 5 neighbor cells means a lumen can start forming
                ifelse not any? link-neighbors and not any? (cells-on neighbors-6) with [ count cells-on neighbors-6 = 6] ;; if I am in a small cluster, not next to a crowded cell, I will initiate lumenogenesis. This is to prevent lumenogenesis initiation at the periphery of a cyst. Is this necessary? Idk.
                [
                  create-cell-links-with (cells-on neighbors-6) [set color blue]
                  ifelse pxcor mod 2 = 0
                  [rule5-even] ;; move to matrix next to cell
                ...
                [
                  ;; cell with links and only cells and matrix neighbors (probably a cell
                  ;; in a thick cyst)
                ]
              ]
            ]
            [
              ifelse count matrix-on neighbors-6 = 0 and count cells-on neighbors-6 > 0 ;; two types of neighbors: cells and empty space
              [
                ifelse not any? link-neighbors
                [
                  ;; put something similar to axiom 4 from Grant
                ]
                [ ;; this rules is for clustering, but I have to validate whether or not it is necessary
                  ifelse pxcor mod 2 = 0
                  [rule9-even]
                  [rule9-odd]
                  ; rule9 ;; move to an empty patch in the neighborhood with the most cells around
                  create-cell-links-with (cells-on neighbors-6) [set color blue]
                ]
              ]
              [
                ifelse count cells-on neighbors-6 = 0 and count matrix-on neighbors-6 > 0 ;; two types of neighbors: matrix and empty space
                [
                  ifelse not any? link-neighbors ;; if you only have matrix and empty space around you, you die of loneliness JK
                  [die]
                  [ ;; in this case I have started lumenogenesis and am linked to other cells, so I will divide and the new cell will occupy matrix next to the lumen
                    ifelse can-divide? = True
                    [
                      set num-divisions num-divisions + 1
                      if num-divisions >= max-divisions [set can-divide? False]
...
;                        create-cell-links-with (cells-on neighbors-6) [set color blue]
                        ifelse pxcor mod 2 = 0
                        [rule3-even] ;; move to matrix next to empty space
                        [rule3-odd]
                        refresh-neighbors
                      ]
                    ]
                ...
                [
                  ...
                  ifelse not any? link-neighbors
                  [
                    ;; cell with three types of neighbor but no links...not possible?
                  ]
                  [
                    ifelse count matrix-on neighbors-6 >= 2 and count cells-on neighbors-6 > 0 ;; and (count matrix-on neighbors-6 + count cells-on neighbors-6) < 5
                    [
                      ifelse can-divide? = True
                      [
                        refresh-neighbors
                        set num-divisions num-divisions + 1
                        if num-divisions >= max-divisions [set can-divide? False]
                        ifelse count (matrix-on neighbors-6) with [count turtles in-radius 1.5 <= 6] > 0 ;; are there any matrix turtles next to empty space?
                        [
                          hatch-cells 1
                          [
                            create-cell-link-with myself [set color blue]
                            move-to one-of (matrix-on neighbors-6) with [count turtles in-radius 1.5 <= 7] ; move to matrix next to empty space
                            ask matrix-here [die]
                            create-cell-links-with (cells-on neighbors-6) [set color blue]
                          ]
                        ]
                        [
                          ...
                          [
                            create-cell-link-with myself [set color blue]
;                            create-cell-links-with (cells-on neighbors-6) [set color blue]
                            move-to one-of matrix-on neighbors-6 with [count cells in-radius 1.5 > 1]
                            ask matrix-here [die]
                            create-cell-links-with (cells-on neighbors-6) [set color blue]
                          ]
                        ]
                      ...
                      [
                        ;; cell linked to neighbors that can no longer divide
                        ;; do nothing
                      ]
                    ]
                    [
                      ;; only one matrix
                      if count matrix-on neighbors-6 = 1 and count cells-on neighbors-6 > 0
                      [move-to one-of matrix-on neighbors-6
                      ask matrix-here [die]
                      create-cell-links-with (cells-on neighbors-6) [set color blue]
                      ]
...
to push-cell-to-matrix
  let movable-cells (cells-on neighbors-6) with [count matrix in-radius 1.5 >= 1]
  ifelse count (cells-on neighbors-6) with [count matrix in-radius 1.5 >= 1] >= 1
  [
    move-to one-of (cells-on neighbors-6) with [count matrix in-radius 1.5 >= 1]
    ifelse pxcor mod 2 = 0 [
      set neighbors-6 turtles-on patches at-points [[0 1] [1 0] [1 -1] [0 -1] [-1 -1] [-1 0]]
    ][
      set neighbors-6 turtles-on patches at-points [[0 1] [1 1] [1  0] [0 -1] [-1  0] [-1 1]] ]
    ...
    [
      move-to one-of (matrix in-radius 1.5 with [count cells in-radius 1.5 > 1])
      ask matrix-here [die]
    ]
  ]
  [die]
...
to rule2-even
  ifelse any? cells-on patch-at 0 1 and not any? cells-on patch-at 0 -1
  [
    move-to patch-at 0 -1
    set ycor ycor - 0.5
    ask matrix-here [die]
  ]
  [ifelse any? cells-on patch-at 1 0 and not any? cells-on patch-at 1 0
    [
      move-to patch-at -1 -1
      ask matrix-here [die]]
    [ifelse any? cells-on patch-at 1 -1 and not any? cells-on patch-at -1 0
      [
        move-to patch-at -1 0
        ask matrix-here [die]
      ]
      [ifelse any? cells-on patch-at 0 -1 and not any? cells-on patch-at 0 1
        ...
        [ifelse any? cells-on patch-at -1 -1 and not any? cells-on patch-at 1 0
          ...
          [
            ifelse not any? cells-on patch-at 1 -1
            [
            move-to patch-at 1 -1
            ask matrix-here [die]
            ]
...
to rule2-odd ;; only one neighbor with no other neighbors - forma a link and move opposite leaving a lumen
  ...
  [ifelse any? cells-on patch-at 1 1 and not any? cells-on patch-at -1 0
    ...
    [ifelse any? cells-on patch-at 1 0 and not any? cells-on patch-at -1 1
      [
        move-to patch-at -1 1
        set ycor ycor - 0.5
        ask matrix-here [die]
      ...
      [ifelse any? cells-on patch-at 0 -1 and not any? cells-on patch-at 0 1
        ...
        [ifelse any? cells-on patch-at -1 0 and not any? cells-on patch-at 1 1
          ...
          [
            ifelse not any? cells-on patch-at 1 0
            [
              move-to patch-at 1 0
              set ycor ycor - 0.5
              ask matrix-here [die]
...
to rule9-odd
  let my-neighbors (count (cells-on neighbors-6) + 1)
  ifelse not any? turtles-on patch-at 0 1 and ([count cells in-radius 1.5] of patch-at 0 1 > my-neighbors)
    [move-to patch-at 0 1]
  [
    ifelse not any? turtles-on patch-at 1 1 and ([count cells in-radius 1] of patch-at 1 1 > my-neighbors)
    [move-to patch-at 1 1
      set ycor ycor - 0.5
    ]
    [
      ifelse not any? turtles-on patch-at 1 0 and ([count cells in-radius 1] of patch-at 1 0 > my-neighbors)
      [move-to patch-at 1 0
        set ycor ycor - 0.5
      ]
      [
        ...
        [
          ...
          [
            if not any? turtles-on patch-at -1 1 and ([count cells in-radius 1] of patch-at -1 1 > my-neighbors)
            [move-to patch-at -1 1
              set ycor ycor - 0.5
            ]
          ]
...
to rule9-even ;; 2 Types: Cell and Free
  let my-neighbors (count (cells-on neighbors-6) + 1)
  ifelse not any? turtles-on patch-at 0 1 and not any? turtles-on patch-at 0 0.5 and (([count cells in-radius 1] of patch-at 0 1 + [count cells in-radius 1] of patch-at 0 0.5) > my-neighbors)
  [
    move-to patch-at 0 1
    set ycor ycor - 0.5
  ]
  [
    ifelse not any? turtles-on patch-at 1 0 and ([count cells in-radius 1.5] of patch-at 1 0 > my-neighbors)
    [move-to patch-at 1 0]
    [
      ifelse not any? turtles-on patch-at 1 -1 and ([count cells in-radius 1.5] of patch-at 1 -1 > my-neighbors)
      [move-to patch-at 1 -1]
      [
        ifelse not any? turtles-on patch-at 0 -1 and not any? turtles-on patch-at 0 -0.5 and (([count cells in-radius 1] of patch-at 0 -1 + [count cells in-radius 1] of patch-at 0 -0.5) > my-neighbors)
        [move-to patch-at 0 -1
        ...
        [
          ...
          [move-to patch-at -1 -1]
          [
            if not any? turtles-on patch-at -1 0 and ([count cells in-radius 1.5] of patch-at -1 0 > my-neighbors)
            [move-to patch-at -1 0]
          ]
        ]
      ]
