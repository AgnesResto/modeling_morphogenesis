...
;  if (pycor > min-pycor) and (pycor < max-pycor) and (pxcor > min-pxcor) and (pxcor < max-pxcor)
;  [
;    ifelse (pxcor mod 2 = 0) and (count cells-here = 1)
;    [
;      ;; procedure for cells in even xcoordinates
;      ifelse not any? cells-on neighbors-6
;      [
;        ifelse count matrix-on neighbors-6 = 6
;        [
;          ;; what happens when all the neighbors are matrix?
;          hatch-matrix 1 [set color white]
;          die
;        ]
;        [
;          ;; what happens when there are no cells in the neighborhood and no matrix neighbors?
;          ifelse (not any? cell-links) or (count matrix-on neighbors-6 = 0)
;          [
;            die
;          ]
;          [
;            ifelse can-divide? = True
;            ;; this is a cell that has formed a lumen with one other cell
;            [
;              set num-divisions num-divisions + 1
;              ;; if I put the can-divide? = False before the hatching, the new cell won't be able to divide anymore
;              if num-divisions >= max-divisions [set can-divide? False]
;              hatch-cells 1
;            [
;              create-cell-link-with myself [set color blue]
...
;            ]
;            [
;              ;; what happens when the cell can no longer divide? rule 7 - move to matrix close to other cells
;              rule7
;            ]
;          ]
;        ]
;      ]
;      [
;        ;; what happens when there are cells on neighboring spaces
;        ifelse (count cells-on neighbors-6 = 1)
;        [
;          ifelse count matrix-on neighbors-6 = 5
;          [;; what happens when the cells is surrounded by matrix and one other cell that is not connected to any other cells?
;           ;; it will form a lumen (i.e. move to have an empty space between them)
;            if (cell-link-neighbor? (one-of cells-on neighbors-6) = False) and (not any? (cells-on neighbors-6) with [count cells-on neighbors-6 > 1]) and (not any? (cells-on neighbors-6) with [cell-link-with (one-of cells-on neighbors-6) = True])
;            [
;              create-cell-links-with cells-on neighbors-6 [set color blue]
;              rule2-even
;              ifelse pxcor mod 2 = 0 [
...
;          ]
;          [
;            ifelse (count (matrix-on neighbors-6) = 4) and (count cells-on neighbors-6 = 1)
;            [
;              set num-divisions num-divisions + 1
;              if num-divisions >= max-divisions [set can-divide? False]
;              hatch-cells 1
...
;            ]
;            [
;              ;; what happens when there is only one cell and no matrix- rule 6
;              ifelse count matrix-on neighbors-6 = 0
;              [
;                rule6-even
;              ]
...
;                ifelse count matrix-on neighbors-6 = 1
;                  [
;                    ;; rule 8 if the matrix is next to the cell - move to matrix
;                    if one-of matrix-on neighbors-6 with [count cells in-radius 1.5 > 1]
;                    [
;                      move-to one-of matrix-on neighbors-6
;                    ]
;                  ]
;                  [
;                    ;; what happens when there are two or three matrix?
;                  ]
;              ]
;            ]
;          ]
;        ]
;        [
;          ;; what happens when there are more than two cells and matrix? This has to do with cluster dynamics
;          ifelse count cells-on neighbors-6 <= 3 and count matrix-on neighbors-6 >= 2 and (count cells-on neighbors-6 + count matrix-on neighbors-6) < 6
;          [
;
;            if num-divisions >= max-divisions [set can-divide? False]
...
;            ]
;            [
;              ;; what happens when there are more than 3 cells on the neighbors?
;
;            ]
;          ]
;        ]
...
;    ]
;    [
;      ;; procedure for cells in odd xcoordinates
;      ifelse not any? cells-on neighbors-6
;      [
;        ifelse count matrix-on neighbors-6 = 6
;        [
...
;        ]
;        [
;          ;; what happens when there are no cells in the neighborhood and some matrix neighbors?
;          ifelse (not any? cell-links) or (count matrix-on neighbors-6 = 0)
;          [
;            die
;          ]
...
;              ]
;              [
;                ;; what happens when the cell has 4 matrix neighbors and can no longer divide?
;              ]
;            ]
;            [
;              ;; what happens when there is only one neighbor cell and no matrix- rule-6
;                ifelse count matrix-on neighbors-6 = 0
;                [
;                  rule6-odd
;                ]
...
to rule7 ;; if cell is connected but only has matrix and empty neighbors,
